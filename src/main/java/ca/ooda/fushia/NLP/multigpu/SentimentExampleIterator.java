    ...
    private DataSet nextDataSet(int num) throws IOException {
        ...
        for( int i=0; i<num && cursor<totalExamples(); i++ ){
            ...
            if(cursor % 2 == 0){
                ...
                String review = FileUtils.readFileToString(positiveFiles[posReviewNumber]).replaceAll("#", "");
                reviews.add(review);
                positive[i] = true;
            } else {
                //Load negative review
                int negReviewNumber = cursor / 2;
                String review = FileUtils.readFileToString(negativeFiles[negReviewNumber]).replaceAll("#", "");
                reviews.add(review);
                positive[i] = false;
            }
            cursor++;
